...
from config import (EIASS_PROFILE, IS_LITE_PROFILE, JOB_RESULT_PAGE_LIMIT, LITE_MAX_CANDIDATES,
                    LITE_RESULT_LIMIT, LITE_SNIPPET_CHARS, LITE_TOOLS,
                    MAX_DOCUMENT_SCAN_BATCH_SIZE, MAX_SCAN_BATCH_SIZE,
...
# ── 실행 프로필에 따른 도구 노출 ──
# MCP 도구의 이름·설명·스키마는 대화마다 통째로 모델 컨텍스트에 실린다. 도구가 늘수록,
# 설명이 길수록 사용자가 한 마디 하기 전에 소비되는 토큰이 커진다(실측 full=20개 ≈16,800토큰).
# 무료 플랜은 컨텍스트·메시지 한도가 빡빡해 이 무게만으로 실사용이 어려워진다.
# lite 프로필은 (1) 도구를 LITE_TOOLS로 제한하고 (2) 설명을 첫 문단만 남겨 ≈5,900토큰으로 낮춘다.
# 설명을 자동으로 잘라내므로 짧은 설명을 따로 관리할 필요가 없다(이중 관리로 인한 불일치 방지).
def _apply_lite_limits(fn):
    """경량 프로필의 상한을 도구 호출 직전에 적용한다.

    도구마다 손으로 넣지 않고 여기서 한 번에 거는 이유: 상한이 필요한 인자(max_candidates 등)를
    쓰는 도구가 여럿이라 하나라도 빠뜨리면 그 경로만 조용히 상한을 우회한다. functools.wraps로
    원본 시그니처가 보존되므로 FastMCP가 만드는 입력 스키마는 그대로다(검증 완료).

    후보 수는 거부하고, 결과 페이지/스니펫 길이는 조용히 낮춘다 — 앞은 "얼마나 조사했는가"라
    사용자가 알아야 하지만, 뒤는 "한 번에 얼마나 보여주는가"라 페이지를 넘기면 그만이다.
    """
    @functools.wraps(fn)
    def wrapper(*args, **kwargs):
        requested = kwargs.get('max_candidates')
        if isinstance(requested, int) and requested > LITE_MAX_CANDIDATES:
            return _lite_candidate_limit_error(requested)
        for name, cap in (('result_limit', LITE_RESULT_LIMIT), ('snippet_chars', LITE_SNIPPET_CHARS)):
...
def tool():
    """프로필에 맞게 도구를 등록한다. full에서는 mcp.tool()과 완전히 동일하게 동작한다."""
    def decorator(fn):
        if not IS_LITE_PROFILE:
            return mcp.tool()(fn)
        if fn.__name__ not in LITE_TOOLS:
            return fn  # 등록하지 않음 — 이 프로필에서는 도구 목록에 나타나지 않는다
        doc = (fn.__doc__ or '').strip()
        summary = doc.split('\n\n')[0].strip() if doc else None
        return mcp.tool(description=summary or None)(_apply_lite_limits(fn))
    return decorator


def _lite_candidate_limit_error(requested):
    """후보 상한 초과는 조용히 잘라내지 않고 거부한다.

    말없이 일부만 훑으면 사용자가 전수 조사한 것으로 오해하는데, 이 도구는 조사 완전성이
    결과의 신뢰도를 좌우한다. 무엇이 왜 막혔고 어떻게 하면 되는지 알려준다.
    """
    return {'error': (f'무료 플랜용 경량 모드에서는 한 번에 최대 {LITE_MAX_CANDIDATES}건까지만 '
                      f'조사할 수 있습니다(요청: {requested}건). 검색 조건을 좁혀 후보를 줄이거나, '
                      f'전체 모드(EIASS_PROFILE=full)로 실행하세요.'),
            'profile': EIASS_PROFILE, 'lite_max_candidates': LITE_MAX_CANDIDATES}
