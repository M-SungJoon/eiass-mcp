...
def eiass_start_document_keyword_scan(
    ...
    **최종 보고 형식(필수)**: 스캔이 done이 되면 (1) `사업명 | eia_cd | 원문 파일명 |
    ...
    now = time.time()
    job = {
        'status': 'queued', 'checked': 0, 'candidates_total': None,
        ...
        'current_phase': 'queued', 'created_at': now, 'updated_at': now,
    }
    with _jobs_lock:
        _cleanup_jobs_locked(_jobs)
        _jobs[job_id] = job

    kwargs = dict(text_queries=query_list, max_candidates=batch_size,
                   audit_sample_size=audit_sample_size, **common_kwargs)
...
def eiass_get_scan_status(job_id: str, include_matches: bool = False,
    ...
    **최종 보고 형식(필수)**: 스캔이 done이 되면 보고/CSV는 매칭된 사업만이 아니라 이번에
    ...
    with _jobs_lock:
        ...
        if include_matches:
            if result_offset < 0 or result_limit <= 0:
                return {'error': 'result_offset은 0 이상, result_limit은 1 이상이어야 합니다.'}
            end = result_offset + min(result_limit, 500)
            result['matches'] = list(job['matches'][result_offset:end])
            result['skipped'] = list(job['skipped'][result_offset:end])
            result['checked_no_match'] = list(job['checked_no_match'][result_offset:end])
            result['result_offset'] = result_offset
            result['result_limit'] = min(result_limit, 500)
    return result
...
def eiass_start_spatial_scan(
    ...
    **최종 보고 형식(필수)**: 스캔이 done이 되면 스캔한 사업 "전체"(scanned)를 기준으로
    ...
    job = {
        ...
        'spatial_failures': [], 'current_phase': 'queued', 'created_at': now, 'updated_at': now,
    }
    with _spatial_jobs_lock:
        _cleanup_jobs_locked(_spatial_jobs)
    ...
    kwargs = dict(radius_m=radius_m, designations=designation_list, allow_admin_fallback=allow_admin_fallback,
                  max_candidates=batch_size, **common_kwargs)
    ...
    **최종 보고 형식(필수)**: done이 되면 scanned(스캔한 전체) 기준으로 보고하라 — matches만
    ...
    with _spatial_jobs_lock:
        ...
        result = {
            ...
            'spatial_failure_count': len(job['spatial_failures']),
            'error': job['error'],
            'current_phase': job['current_phase'],
            'updated_at': job['updated_at'],
        }
        if include_results:
            ...
            result['scanned'] = list(job['scanned'][result_offset:end])
            result['matches'] = list(job['matches'][result_offset:end])
            result['geocode_failures'] = list(job['geocode_failures'][result_offset:end])
            result['spatial_failures'] = list(job['spatial_failures'][result_offset:end])
            result['result_offset'] = result_offset
            result['result_limit'] = min(result_limit, 500)
    return result
